...
export const providersApi = {
  listProviders: (): Promise<ProviderInfo[]> => request('/api/providers/list', undefined, 'GET'),

  saveApiKey: (provider: string, key: string): Promise<void> => request('/api/keys/save', {provider, key}),

  hasApiKey: (provider: string): Promise<boolean> => request('/api/keys/has', {provider}),

  deleteApiKey: (provider: string): Promise<void> => request('/api/keys/delete', {provider}),

  testProviderConnection: (provider: string): Promise<ProviderTestResult> => request('/api/providers/test', {provider}),
  ...
  getGitHubUser: (): Promise<GitHubUser | null> => request('/api/providers/github/user', undefined, 'GET'),
  ...
  getCopilotUser: (): Promise<GitHubUser | null> => request('/api/providers/copilot/user', undefined, 'GET'),
}
