...
const allTypes = new Set<Block['type']>([
    'ACTION', 'LOOP', 'CONDITION', 'SUBFLOW', 'ERROR_HANDLER', 'COMMENT',
    'VARIABLE', 'WAIT', 'BLOCK', 'SWITCH', 'ELSE', 'CASE', 'DEFAULT', 'END', 'UNKNOWN',
])
...
describe('flattenTreeRows', () => {
    ...
    it('filters out blocks not in visibleTypes', () => {
        const sf = makeSubflow('sf1', [
            makeBlock('a1', 'ACTION'),
            makeBlock('c1', 'COMMENT'),
        ])
        const doc = makeDoc(sf)
        const result = flattenTreeRows(doc, {
            ...baseOptions,
