...
type DialogState = ({kind: 'confirm'} & ConfirmOptions) | ({kind: 'prompt'} & PromptOptions)
      ...
      >
        {state && (
          <div className="space-y-3">
            {state.title && <h2 className="text-base font-semibold text-text-primary">{state.title}</h2>}
            {state.kind === 'confirm' ? (
              ...
              <div className="space-y-2">
                ...
                {state.label && <label className="block text-sm font-medium text-text-primary">{state.label}</label>}
                <input
                  autoFocus
                  value={inputValue}
