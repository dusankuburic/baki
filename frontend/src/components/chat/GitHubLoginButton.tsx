...
export default function GitHubLoginButton({onAuthComplete}: Props) {
  ...
  const startAuth = useCallback(async () => {
    try {
      const resp = await providersApi.startGitHubAuth()
      if (!resp) return
      ...
      const poll = async () => {
        ...
        try {
          const result = await providersApi.pollGitHubAuth(resp.device_code)
          if (result.status === 'success') {
            pollingRef.current = false
            setState('configured')
            const user = await providersApi.getGitHubUser()
  ...
  const disconnect = useCallback(async () => {
    try {
      await providersApi.revokeGitHubAuth()
    } catch (e) { console.error('Revoke GitHub auth failed:', e) }
    setState('unconfigured')
    setUsername('')
  }, [])
  ...
  switch (state) {
    case 'unconfigured':
      return (
        <button
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#24292e] hover:bg-[#2f363d] text-white text-sm transition-colors"
          onClick={startAuth}
        >
          <Github size={16} />
          Sign in with GitHub
