          ...
          return (
            <div key={f.id} className="px-3 py-2 hover:bg-surface-2/50 transition-colors">
              <div className="flex items-start gap-2">
                <Icon
                  size={12}
                  className={clsx('mt-0.5 shrink-0', severityColor[f.severity] ?? 'text-text-tertiary')}
                />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {f.category && (
                      <span
                        className={`text-2xs font-bold uppercase tracking-wider px-1 py-0.5 rounded ${categoryBadgeClass(f.category)}`}
                      >
                        {f.category}
                      </span>
                    )}
                  ...
                  {f.suggestion && <p className="text-2xs text-emerald-400/80 mt-1 line-clamp-1">{f.suggestion}</p>}
