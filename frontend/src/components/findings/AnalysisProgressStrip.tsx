...
export default function AnalysisProgressStrip({progress}: {progress: {current: number; total: number; ruleName: string}}) {
  ...
  return (
    <div
      ...
      role="status"
      aria-label={pct === null ? 'Analysis in progress' : `Analysis in progress, ${pct}%`}
    >
      <Spinner size={12} />
      <span className="text-2xs text-text-secondary tabular-nums whitespace-nowrap">
        {pct === null ? 'Analyzing…' : `Analyzing… ${pct}%`}
