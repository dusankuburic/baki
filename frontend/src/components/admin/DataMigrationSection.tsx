...
export const DataMigrationSection: React.FC<{
  ...
  <section className="bg-surface-2 border border-border-default rounded-xl overflow-hidden">
    <div className="px-5 py-3 border-b border-border-subtle flex items-center gap-2">
      ...
      {status?.status && (
        <span className={clsx('ml-auto px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase', migrationStatusClass(status.status))}>
          {status.status}
        </span>
      )}
    </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-surface-3 border border-border-subtle rounded-lg p-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="flex items-center gap-1">
              {status.result.SettingsMoved ? <CheckCircle2 size={14} className="text-semantic-success" /> : <XCircle size={14} className="text-semantic-error" />}
              <span className="text-sm font-semibold text-text-primary">{status.result.SettingsMoved ? 'Moved' : 'Not moved'}</span>
            </span>
          </div>
