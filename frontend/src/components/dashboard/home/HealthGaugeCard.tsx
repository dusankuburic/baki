...
export function HealthGaugeCard({
  ...
  // to explain it.
  const zeroCause =
    score === 0 && errors + warnings + info > 0
      ? [errors && `${errors} errors`, warnings && `${warnings} warnings`, info && `${info} info`]
          .filter(Boolean)
          .join(' · ')
      : null
      ...
      {!overview.healthAvailable ? (
        ...
        <div className="relative h-48">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-4xl font-black font-mono tabular-nums" style={{color: fill}}>
              {score}
            </span>
            <span className="text-2xs uppercase tracking-widest text-text-tertiary mt-1">of 100</span>
            {zeroCause && <span className="text-2xs text-text-tertiary mt-1">{zeroCause}</span>}
          </div>
        </div>
      )}
    </CardShell>
