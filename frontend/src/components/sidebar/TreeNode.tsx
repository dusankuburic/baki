...
function highlightText(text: string, query: string | undefined, highlights?: Highlight[]): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="bg-brand-500/30 text-text-primary rounded px-0.5">{text.slice(idx, idx + query.length)}</mark>
...
function TreeNode({
  ...
  return (
    ...
    >
      {isViewportVisible && <div className="absolute left-0 top-0 bottom-0 w-1 bg-brand-500/50" />}
      ...
      {row.hasChildren && !isExpanded && !findingCount && (
        <span className="flex-shrink-0 text-2xs text-text-tertiary tabular-nums mr-2">{row.childCount}</span>
      )}
    </div>
  )
}
