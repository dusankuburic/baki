...
import {FileText, FolderOpen, Search, ArrowDownAZ, ArrowDownWideNarrow, Copy, ExternalLink, RotateCw} from 'lucide-react'
...
export default function FileList({files, selectedFilePath, folderName, onSelectFile, onRevealFile, onReloadFile}: FileListProps) {
    ...
    const visibleFiles = useMemo(() => {
        ...
        const sorted = [...filtered].sort((a, b) =>
            sortMode === 'size' ? b.size - a.size : a.name.localeCompare(b.name))
        return sorted
    ...
    const clampedFocusedIndex = visibleFiles.length === 0 ? -1 : Math.min(Math.max(focusedIndex, 0), visibleFiles.length - 1)
    ...
    const menuItems: ContextMenuItem[] = ctxMenu ? [
        {
            label: 'Copy path',
            icon: Copy,
            onClick: () => { writeClipboard(ctxMenu.file.path) },
        },
        ...(onRevealFile ? [{
            ...
            {files.length > 4 && (
                <div className="flex items-center gap-1.5 px-3 py-1.5 border-b border-border-subtle">
                    ...
                    <button
                        ...
                        title={sortMode === 'name' ? 'Sorted by name — click to sort by size' : 'Sorted by size — click to sort by name'}
                    >
                ...
                {visibleFiles.length === 0 ? (
                    ...
                    return (
                        <button
                            ...
                            onClick={() => { setFocusedIndex(idx); onSelectFile(file.path) }}
                            onContextMenu={(e) => {
                                e.preventDefault()
                                setFocusedIndex(idx)
