  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: selected ? `color-mix(in srgb, ${color} 8%, var(--surface-1))` : 'var(--surface-1)',
        opacity: 0.85,
      }}
      onClick={onClick}
    >
      ...
      <Icon size={13} style={{color, flexShrink: 0}} strokeWidth={2.5} />

      <span className="text-[11px] font-semibold uppercase tracking-widest" style={{color}}>
        {label}
      </span>

      <span className="text-2xs text-text-tertiary ml-auto">L{block.lineNumber}</span>
    </div>
  )
}
